...
echo "This script will let you define the Mantl deployment for your bot"
echo

echo "What is your Mantl Control address?"
read control_address
echo
echo "What is the Mantl username?"
read mantl_user
echo
echo "What is the Mantl password?"
read -s mantl_password
echo
echo "What is the app domain?"
read mantl_domain
echo
...
echo "Into what folder do you want to deploy your bot?"
read deployment_dir
echo
...
if [ $TAG != "" ]
then
    sed -i "" -e "s/latest/$TAG/g" $docker_username-$bot_name.json
fi
...
-d @$docker_username-$bot_name.json \
| python -m json.tool
...
BOT_URL="http://$deployment_dir-$bot_name-$docker_username.$mantl_domain"
...
echo
echo "http://$deployment_dir-$bot_name-$docker_username.$mantl_domain/"
echo
echo "You should be able to send a message to yourself from the bot by using this call"
echo
echo "curl http://$deployment_dir-$bot_name-$docker_username$mantl_domain/hello/<YOUR EMAIL ADDRESS>"
echo
